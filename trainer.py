...
class Trainer:
    def __init__(self, config, model, dataloaders, is_pm=False):
        ...
        self.vocab = config.vocab # CFG 통해 들어오는 vocab 저장 위해
    ...
    def valid_one_epoch(self, stage='valid'):
        ...
        """
        self.model.eval()
        pbar = tqdm(enumerate(self.valid_loader), total=len(self.valid_loader)) # 나중에 valid 접두어 지우기
        outputs_dict = {'output': [], 'pred': [], 'target': [], 'loss': []} # 나중에 preds 같은 식으로 변수명 바꾸기
        # has_attn = False
        total_loss = 0
        ...
        for idx, data_dict in pbar:
            ...
            if self.is_pm:
                # rnn의 attn도 시각화하려면 input_ids를 위로 올리고 packing 사용 시에도 오류 발생 막으려면 배치마다 길이를 맞춰주는 코드 추가 필요
        ...
        epoch_avg_loss = round(total_loss/ len(self.valid_loader.dataset), 4) # 소수 둘째자리에서 반올림하니 early_stopping이 올라가는 경우가 너무 빈번
    ...
    def test(self):
        outputs_dict = self.valid_one_epoch(stage='valid') # wandb에서 valid와 test 성능 비교 위해 통일
        return outputs_dict
